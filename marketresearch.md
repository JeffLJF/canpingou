        ...
        * 阿尔法、贝塔、伽马
            ...
            * 贝塔测试 bata testing
                ...
                * 目的
                    ...
                    * 检查不同类型的网络和硬件下产品的兼容性
            * 伽马测试 gamma testing
                * 在满足特定要求的软件已完成发布的工作后，开始进行伽马测试；
                * 伽马测试后，几乎是上市的最终版本，不在进行功能开发或改进，仅修改限定范围内的代码错误；
                * 除了在开发周期时间极短，上市速度要求极快的高压情况下，伽马测试并不常见；

        * 试销 test marketing
            * 定义：为了观察消费者对产品的反应，将新产品以一种严格控制的方式投放在一个或多个有限区域，针对多个低于，可将不同广告定位或产品定位进行比较
            * 方式
                * 销售波研究
                * 模拟营销测试
                * 控制试销
...
## 多变量分析与多变量方法
* 定义
    * 多变量分析 multivariate analysis 探讨一个结果变量（因变量）与一个或多个预测变量（自变量）之间的关系
* 方法
    * 因子分析 factor analysis 
    * 减少变量的数量
    * 找出变量之间的结构关系
    * 聚类分析 cluster analysis
    * 联系分析 conjoint analysis
